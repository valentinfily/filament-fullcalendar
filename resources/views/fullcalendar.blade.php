...
<x-filament-widgets::widget>
    <x-filament::section>
        ...
        @if ($plugin->isEditable() && $plugin->isDroppable())
            <div id='draggable-events'>
                @foreach ($this->draggableEvents() as $draggableEvent)
                    <x-filament::button outlined class='fc-event'
                        data-event="{{ json_encode($draggableEvent) }}">{{ $draggableEvent['title'] }}</x-filament::button>
                @endforeach
            </div>
        @endif
